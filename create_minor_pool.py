...
def compare_gradients(previous_gradients, current_gradients):
    ...
    abs_differences = [
        abs(prev - curr)
        for prev, curr in zip(previous_gradients[1], current_gradients[1])
    ]
    ...
    for miner_id in range(num_miners):
        ...
        if miner_id > 0:
            for comparison in range(min(num_comparisons, miner_id)):
                ...
                previous_gradients_str = redis_client.hget(
                    f"miners:{previous_miner_id}", "model_gradients"
                )
                current_gradients_str = redis_client.hget(
                    f"miners:{miner_id}", "model_gradients"
                )

                # Check if gradients are not None
                if (
                    previous_gradients_str is not None
                    and current_gradients_str is not None
                ):
                    ...
                    redis_client.hset(
                        f"miners:{miner_id}", f"score_{comparison + 1}", score
                    )
                    print(
                        f"Miner {miner_id} scored {score} in comparison {comparison + 1}"
                    )
                else:
                    print(
                        f"Miner {miner_id} or its previous miner failed to compute gradients."
                    )
